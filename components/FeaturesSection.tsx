  ...
  const localFeatures: FeatureItem[] = features ?? [
    {
      ...
      icon: (
        ...
        <svg className="w-7 h-7 md:w-8 md:h-8" viewBox="0 0 24 24" fill="currentColor" style={{color:'#5A5E4D'}}>
          ...
          {localFeatures.map((feature) => (
            ...
            >
              <div className="mb-2 md:mb-3">
                <div className="w-full flex justify-start">{feature.icon}</div>
                <h3 dir="rtl" className="mt-2 text-base md:text-lg font-bold text-gray-800 text-right" style={{fontFamily: 'var(--font-almarai)'}}>
                  {feature.title}
                </h3>
              </div>
              <p dir="rtl" className="text-[12px] md:text-sm text-gray-600 leading-relaxed text-right" style={{fontFamily: 'var(--font-almarai)'}}>
                {feature.description}
              </p>
            </div>
          ))}
