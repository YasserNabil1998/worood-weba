...
export default function VerifyPage() {
  ...
  // Read phone from URL on client after mount to avoid SSR hydration mismatch
  useEffect(() => {
    try {
            ...
            <div className="px-6 pb-6">
              <form onSubmit={onSubmit} className="space-y-5">
                <div>
                  ...
                  <div className="flex justify-center gap-2">
                    {[0,1,2,3].map((i) => (
                      <input
                        ...
                        onKeyDown={(e)=>{
                          if (e.key === 'Backspace' && !code[(e.target as HTMLInputElement).selectionStart ?? 0] && inputsRef.current[(e.target as any).dataset?.index-1]) {
                            const idx = Number((e.target as any).dataset.index);
                  ...
                  <p className="mt-2 text-[11px] text-gray-500 text-center" suppressHydrationWarning>تم الإرسال إلى {phone || '+966 ••• ••• ••••'}</p>
                ...
                <div className="text-center text-xs text-gray-600 space-y-2">
                  <a href="/login" className="block hover:underline">العودة لتغيير رقم الهاتف</a>
                  <button type="button" className="text-gray-700 hover:underline">إعادة إرسال الرمز</button>
                </div>
              </form>
            </div>
          </div>
