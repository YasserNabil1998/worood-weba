...
interface CartItem { id: number; title: string; size: string; style: string; color: string; total: number }

export default function CheckoutPage() {
  const [items, setItems] = useState<CartItem[]>([]);
  const [address, setAddress] = useState({ city: '', district: '', street: '', landmark: '', phone: '' });
    ...
    <div className="min-h-screen" dir="rtl">
      ...
      <main className="py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-800 mb-6" style={{fontFamily:'var(--font-almarai)'}}>متابعة الدفع</h1>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Form */}
            <div className="lg:col-span-2 space-y-6">
              <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
                <h2 className="text-lg font-semibold mb-3" style={{fontFamily:'var(--font-almarai)'}}>عنوان التوصيل</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input placeholder="المدينة" value={address.city} onChange={(e)=> setAddress({...address, city:e.target.value})} className="rounded-lg border border-gray-200 px-3 py-2" />
                  <input placeholder="الحي" value={address.district} onChange={(e)=> setAddress({...address, district:e.target.value})} className="rounded-lg border border-gray-200 px-3 py-2" />
                  <input placeholder="الشارع" value={address.street} onChange={(e)=> setAddress({...address, street:e.target.value})} className="rounded-lg border border-gray-200 px-3 py-2" />
                  <input placeholder="أقرب معلم" value={address.landmark} onChange={(e)=> setAddress({...address, landmark:e.target.value})} className="rounded-lg border border-gray-200 px-3 py-2" />
                  <input placeholder="رقم الجوال" value={address.phone} onChange={(e)=> setAddress({...address, phone:e.target.value})} className="rounded-lg border border-gray-200 px-3 py-2" />
                </div>
              </section>

              <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
                <h2 className="text-lg font-semibold mb-3" style={{fontFamily:'var(--font-almarai)'}}>طريقة الدفع</h2>
            ...
            <aside className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 h-fit">
              ...
              <ul className="mb-3 divide-y">
                {items.map((it)=> (
                  <li key={it.id} className="py-2 flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold">ريال {it.total}</span>
                  </li>
                ))}
              </ul>
              ...
              <button onClick={placeOrder} className="mt-4 w-full rounded-md bg-[#5A5E4D] text-white py-2">تأكيد الطلب</button>
            </aside>
          </div>
        </div>
      </main>
