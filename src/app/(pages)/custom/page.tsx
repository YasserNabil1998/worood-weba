...
import cardSuggestionsData from "@/data/card-suggestions.json";
...
import { useCartOperations } from "@/hooks/custom/useCartOperations";
import { useCustomBuilderData } from "@/hooks/custom/useCustomBuilderData";
import { useDesignFromUrl } from "@/hooks/custom/useDesignFromUrl";
...
import CustomBuilderHeader from "@/components/custom/CustomBuilderHeader";
import CustomBuilderNavigation from "@/components/custom/CustomBuilderNavigation";
import CustomBuilderSteps from "@/components/custom/CustomBuilderSteps";
import DataLoader from "@/components/shared/DataLoader";
import AOSWrapper from "@/components/common/AOSWrapper";
import { UI_TEXTS } from "@/constants";

const BouquetPreview = lazy(() => import("@/components/custom/BouquetPreview"));
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                <div className="order-2 lg:order-2 lg:col-span-2">
                  ...
                  <AOSWrapper animation="fade-up" delay={150} duration={800}>
                    <CustomBuilderSteps
                      ...
                      onInc={(flowerId: string | number) =>
                        flowerManagement.inc(
                          typeof flowerId === "string" ? Number(flowerId) : flowerId
                        )
                      }
                      onDec={(flowerId: string | number) =>
                        flowerManagement.dec(
                          typeof flowerId === "string" ? Number(flowerId) : flowerId
                        )
                      }
                      qty={(flowerId: string | number) =>
                        flowerManagement.qty(
                          typeof flowerId === "string" ? Number(flowerId) : flowerId
                        )
                      }
                      ...
                      onSetFlowerColor={(flowerId: string, colorId: string) =>
                        flowerManagement.setFlowerColor(flowerId, Number(colorId))
                      }
                      ...
                      onSizeChange={(newSize: string) => {
                        if (["small", "medium", "large", "custom"].includes(newSize)) {
                          flowerManagement.handleSizeChange(
                            newSize as "small" | "medium" | "large" | "custom"
                          );
                        }
                      }}
                      packagingType={state.packagingType}
                ...
                <AOSWrapper animation="fade-left" delay={100} duration={800}>
                  <Suspense
                    fallback={<div className="text-center py-8">جاري تحميل المعاينة...</div>}
                  >
                    <BouquetPreview
                      bouquetImage={state.bouquetImage}
                      total={prices.total}
