...
export default function SocialMediaLinks({ data }: SocialMediaLinksProps) {
  return (
    <div className="mt-4 pt-4 border-t border-gray-200">
      <h4 className="text-lg font-bold text-gray-900 mb-5 text-center">
        {data.title}
      </h4>
        ...
        {data.links.map((link) => {
          ...
          return (
            <a
              ...
              className={`h-12 w-12 rounded-full bg-gray-100 text-gray-700 flex items-center justify-center hover:text-white hover:scale-110 transition-all duration-300 shadow-sm hover:shadow-lg ${colorClass}`}
              title={link.title}
            >
              <IconComponent />
            </a>
