...
export function useProductDetails(productId: string) {
        ...
        async function fetchProduct() {
            await withLoading(async () => {
                try {
                    ...
                    const mainImage =
                        PRODUCT_DATA.bouquetImages[
                            parseInt(productId) %
                                PRODUCT_DATA.bouquetImages.length
                        ];
                    const product: Product = {
                        id: data.id || parseInt(productId) || 0,
                        title: data.title || "منتج غير محدد",
                        price: Math.round(
                            (data.price || 0) * PRODUCT_DATA.priceMultiplier
                        ),
                        image: mainImage,
                        images: [
                            mainImage,
                            PRODUCT_DATA.productImages[
                                parseInt(productId) %
                                    PRODUCT_DATA.productImages.length
                            ],
                            PRODUCT_DATA.productImages[
    ...
    const getTotalPrice = () => {
        ...
        const selectedSizeData = PRODUCT_DATA.sizes.find(
            (s) => s.value === options.selectedSize
        );
