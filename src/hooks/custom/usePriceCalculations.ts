...
export function usePriceCalculations({
  ...
  const stylePrice = useMemo(() => getStylePrice(style), [style, bouquetStyles]);
  ...
  const cardPrice = useMemo(() => (includeCard ? config.cardPrice : 0), [includeCard, config]);
  ...
  const subtotal = useMemo(() => {
    ...
    const vasePriceValue =
      packagingType === "vase" && selectedVase
        ...
        : 0;
    const flowersPriceValue = Object.entries(selectedFlowers).reduce((sum, [id, qty]) => {
      const f = flowers.find((x) => x.id === Number(id));
      return sum + (f ? f.price * qty : 0);
    }, 0);
