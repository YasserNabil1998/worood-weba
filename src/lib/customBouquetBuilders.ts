...
export function buildVaseDetails(
    selectedVase: string,
    vases: Vase[]
): Vase | undefined {
...
export function buildDeliveryInfo(
    ...
    return {
        ...
        time: deliveryTime,
        timeLabel:
            deliveryTimes.find((t) => t.value === deliveryTime)?.label || "",
        address: {
            city,
            district,
            street,
        ...
        paymentMethodLabel:
            paymentMethods.find((p) => p.key === payMethod)?.label || "",
    };
}
...
export function buildCustomData(
    ...
    const flowersDetails = buildFlowersDetails(
        input.selectedFlowers,
        sources.flowers
    );
    ...
    const occasionDetails = buildOccasionDetails(
        input.occasion,
        sources.occasions
    );
